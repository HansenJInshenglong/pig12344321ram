...
open class LBXScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate {
    ...
    open func captureImage() {
        ...
        stillImageOutput.captureStillImageAsynchronously(from: stillImageConnection, completionHandler: {[weak self] (imageDataSampleBuffer, _) -> Void in
            self?.stop()
            guard let arrayResult = self?.arrayResult else {
                return
            }
            if let imageDataSampleBuffer = imageDataSampleBuffer,
                ...
                for idx in 0 ... arrayResult.count - 1 {
                    self?.arrayResult[idx].imgScanned = scanImg
                }
            }
            self?.successBlock(arrayResult)
        })
    }

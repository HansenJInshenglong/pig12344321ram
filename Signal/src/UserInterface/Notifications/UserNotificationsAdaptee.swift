...
//                    owsFailDebug("error was unexpectedly nil")
    ...
    func shouldPresentNotification(category: AppNotificationCategory, userInfo: [AnyHashable: Any]) -> Bool {
        ...
        if let baseTab = UIApplication.shared.frontmostViewControllerIgnoringAlerts as? UITabBarController {
            
            let vc =  baseTab.selectedViewController?.findFrontmostViewController(true);
            
            if vc is ConversationViewController || vc is SessionVC {
                return false;
            }
            
        }

        guard let conversationViewController = UIApplication.shared.frontmostViewControllerIgnoringAlerts as? ConversationViewController else {
            return true
        }

...
- (void)observeNotifications
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(applicationWillResignActive:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateSessionSticks) name:@"kNotification_pigram_update_stick" object:nil];
...
//                                               object:nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(localProfileDidChange:)
                                               ...
                                               object:nil];
#pragma mark 添加好友申请的通知  hansen
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onInvitedByFriend:) name:@"kNotification_Contact_Action" object:nil];
}
...
- (void)onInvitedByFriend:(NSNotification *)info {
    
    OWSAssertIsOnMainThread();

    NSNumber *action = info.object;
    switch (action.intValue) {
        case 1:
            [OWSAlerts showAlertWithTitle:@"收到好友申请"];
            break;
    case 2:
            [OWSAlerts showAlertWithTitle:@"邀请被同意"];

        break;
    case 3:
            [OWSAlerts showAlertWithTitle:@"邀请被拒绝"];
        break;
        default:
            break;
    }
...
- (void)loadView
{
    ...
    ReminderView *outageView = [ReminderView
        nagWithText:NSLocalizedString(@"网络连接已中断，请检查你手机的网络！正在尝试重新连接...", @"Label warning the user that the Signal service may be down.")
          tapAction:nil];
...
//    emptyInboxLabel.lineBreakMode = NSLineBreakByWordWrapping;
    ...
    upLabel.textColor = [UIColor hex:@"#687683"];
    upLabel.text = NSLocalizedString(@"您还没有会话", nil);
    UILabel *subLabel = [UILabel new];
    subLabel.font = [UIFont systemFontOfSize:12];
    subLabel.textColor = [UIColor hex:@"#687684"];
    subLabel.text = NSLocalizedString(@"请点击右上角“···”图标来创建您的聊天会话", nil);
...
#pragma mark 更新所有的置顶会话
- (void)updateSessionSticks {
    
    [PigramNetworkMananger pg_getMyInfoWithSuccess:^(id _Nullable response) {
        
        if (response) {
            NSArray *sticks = response[@"sticks"];
            if (sticks.count > 0) {
                [SSKEnvironment.shared.databaseStorage asyncWriteWithBlock:^(SDSAnyWriteTransaction * _Nonnull write) {
                    NSArray<TSThread *> *allThreads = [TSThread anyFetchAllWithTransaction:write];
                    for (TSThread *thread in allThreads) {
                        NSDictionary *result =  nil;
                        for (NSDictionary *dict in sticks) {
                            NSString *id = dict[@"id"];
                            if ([id isEqualToString:thread.recipientAddresses.firstObject.phoneNumber]) {
                        ...
                        uint64_t timestamp = [result[@"timestamp"] unsignedLongLongValue];
                        [thread anyUpdateWithTransaction:write block:^(TSThread * _Nonnull _value) {
                            if (result) {
                                _value.tx_top = true;
                                _value.tx_top_date = timestamp;
                            } else {
                                _value.tx_top = false;
                            }
                        }];
...
//    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUALaa_TO(11, 0)) {
...
//        settingsButta'aon = [[UIBarButtonItem alloc] initWithImage:image
//                                                          style:UIBarButtonItemStylePlain
//                                                         target:self
//                                                         action:@selector(settingsButtonPressed:)
...
//    FindByPhoneNumberViewController *vc =
//    [[FindByPhoneNumberViewController alloc]
//                initWithDelegate:nil
//                      buttonText:@"添加好友"
//        requiresRegisteredNumber:true];
...
- (BOOL)extracted {
    return [self updateHasArchivedThreadsRow];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [AppReadiness runNowOrWhenAppDidBecomeReady:^{
        if (TSAccountManager.sharedInstance.isRegisteredAndReady) {
            [AppEnvironment.shared.messageFetcherJob run];
        }
    }];
    [self updateSessionSticks];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (section) {
        ...
        case HomeViewControllerSectionConversations: {
            ...
            if (model.inviteAction != 2 && model.inviteAction > 0) {
                if (model.inviteAction == 1 && !model.inviteActionIsSentByMe) {
                    [self handleInvitationFromModel:model];
                }
                return;
            }
...
#pragma mark 处理被好友邀请的选择处理

- (void)handleInvitationFromModel:(ThreadViewModel *)model {
    
//    UIAlertController *vc = [UIAlertController alertControllerWithTitle:@"请选择" message:model.lastMessageText preferredStyle:UIAlertControllerStyleActionSheet];
//
...
//
//        [SSKEnvironment.shared.contactsUpdater sendFriendInviteWithRecipient:model.contactAddress action:PigramContactActionAccept success:^{
//        } failure:^(NSError * _Nonnull error) {
//            dispatch_async(dispatch_get_main_queue(), ^{
//
...
//        }];
//    }];
//    UIAlertAction *refuse = [UIAlertAction actionWithTitle:@"拒绝" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
//
//        [SSKEnvironment.shared.contactsUpdater sendFriendInviteWithRecipient:model.contactAddress action:PigramContactActionDecline success:^{
//        } failure:^(NSError * _Nonnull error) {
//            dispatch_async(dispatch_get_main_queue(), ^{
//                [OWSAlerts showAlertWithTitle:error.localizedDescription];
//            });
             ...
             animated:(BOOL)isAnimated
{
    ...
    DispatchMainThreadSafe(^{
        ...
        [conversationVC setHidesBottomBarWhenPushed:true];
        ...
        if (tabBarVC) {
            if (tabBarVC.presentedViewController) {
                [tabBarVC.presentedViewController dismissViewControllerAnimated:false completion:^{
                    UINavigationController *nav = tabBarVC.selectedViewController;
                             
                               [nav setViewControllers:@[nav.viewControllers.firstObject, conversationVC] animated:true];
                }];
            } else {
                UINavigationController *nav = tabBarVC.selectedViewController;
            ...
            [self.navigationController setViewControllers:@[self.navigationController.viewControllers.firstObject, conversationVC] animated:true];
...
- (void)updateViewState
{
    ...
    if ([self.threadMapping numberOfItemsInSection:1] > 0) {
        [self.emptyInboxView setHidden:YES];
    } else {
        [self.emptyInboxView setHidden:false];
    }
}

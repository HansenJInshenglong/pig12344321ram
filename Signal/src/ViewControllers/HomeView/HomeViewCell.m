...
@property (nonatomic) DYUnreadView *unreadView;

@property (nonatomic) UIImageView *stickView;
...
- (void)commontInit
{
    ...
    self.stickView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"pigram-session-stick"]];
    [self.stickView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.offset(11);
    }];
    self.unreadView = [[DYUnreadView alloc] init];
    ...
    bottomRowView.userInteractionEnabled = true;
    ...
    [self.unreadView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.center.equalTo(self.stickView);
    }];
    self.stickView.hidden = true;
}
               ...
               overrideDate:(nullable NSDate *)overrideDate
{
    ...
    if (self.thread.threadRecord.tx_top_date > 0) {
        //会话被置顶
        self.contentView.backgroundColor = [UIColor groupTableViewBackgroundColor];
        self.stickView.hidden = false;
    } else {
        ...
        if ([self.thread.lastMessageForInbox isKindOfClass:[TSOutgoingMessage class]]) {
            ...
            switch (messageStatus) {
                ...
                case MessageReceiptStatusSkipped:
                    statusIndicatorImage = [UIImage imageNamed:@"pigram-session-sent"];
                    messageStatusViewTintColor = [UIColor hex:@"#6cc96e"];
                    break;
                case MessageReceiptStatusDelivered:
                    statusIndicatorImage = [UIImage imageNamed:@"pigram-session-sentAndRead"];
                    messageStatusViewTintColor = [UIColor hex:@"#6cc96e"];
                    break;
...
- (NSAttributedString *)attributedSnippetForThread:(ThreadViewModel *)thread isBlocked:(BOOL)isBlocked
{
    ...
    if (isBlocked) {
        ...
        if (displayableText) {
            if ([thread.lastMessageForInbox isMemberOfClass:TSIncomingMessage.class]) {
                
                TSIncomingMessage *message = (TSIncomingMessage *)thread.lastMessageForInbox;
                
                if (message.isMentionedMe) {
                        [snippetText appendAttributedString:[[NSAttributedString alloc] initWithString:@"有人@了你 " attributes:@{
                            NSFontAttributeName :
                                (hasUnreadMessages ? self.snippetFont.ows_mediumWeight
                                 : self.snippetFont),
                            NSForegroundColorAttributeName :
                                (hasUnreadMessages ? kThemeTextColor
                                 : kThemeTextColor),
                        }]];
                } else if (message.isMentionedAll) {
                    [snippetText appendAttributedString:[[NSAttributedString alloc] initWithString:@"@所有人 " attributes:@{
                        NSFontAttributeName :
                            (hasUnreadMessages ? self.snippetFont.ows_mediumWeight
                             : self.snippetFont),
                        NSForegroundColorAttributeName :
...
- (void)updateNameLabel
{
    ...
    __block NSString *_Nullable name;
    ...
    if (thread.isGroupThread) {
        ...
        if (self.thread.threadRecord.isNoteToSelf) {
            ...
            [SSKEnvironment.shared.databaseStorage readWithBlock:^(SDSAnyReadTransaction * _Nonnull read) {
                OWSUserProfile *user = [OWSUserProfile getUserProfileForAddress:thread.contactAddress transaction:read];

...
@property (nonatomic, strong) MessageBlockView *blockView;
...
- (void)setBlockSendMessage:(BOOL)isBlock {
    
    self.userInteractionEnabled = !isBlock;
    self.blockView.hidden = !isBlock;
...
- (void)setMessageText:(NSString *_Nullable)value animated:(BOOL)isAnimated
{
    ...
    if (value.length > 0) {
        NSString *lastStr = [value substringWithRange:NSMakeRange(value.length - 1, 1)];
        if ([lastStr isEqualToString:@"@"]) {
            [self.inputTextView.inputTextViewDelegate textViewInputMentionText];
        }
    }
...
- (void)lockVoiceMemoUI
{
    ...
    [self.inputTextView insertTextPlaceholderWithSize:CGSizeMake(100, 100)];
}
...
- (MessageBlockView *)blockView {
    
    if (!_blockView) {
        _blockView = [MessageBlockView new];

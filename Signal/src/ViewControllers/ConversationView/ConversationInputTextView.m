...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if (textView.text.length > 0) {
        lastStr = [textView.text substringWithRange:NSMakeRange(textView.text.length - 1, 1)];
    }
    if ([text isEqualToString:@"@"]) {
        [self.inputTextViewDelegate textViewInputMentionText];
    } else if (text.length == 0 && [lastStr isEqualToString:@"@"]) {
        
        [self.inputTextViewDelegate textViewDeleteMentionText];
    }

...
- (void)tapAvatarAction:(UITapGestureRecognizer *)tap{
    if ([self.delegate respondsToSelector:@selector(txTapAvatarconversationCell:Item:)]) {
        [self.delegate txTapAvatarconversationCell:self Item:self.viewItem];
    }
}
...
- (void)commontInit
{
    ...
    UITapGestureRecognizer *avatarTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapAvatarAction:)];
...
- (void)handleLongPressGesture:(UILongPressGestureRecognizer *)sender
{
    ...
    if (self.avatarView) {
        CGPoint avatarPoint = [self.messageView convertPoint:locationInMessageBubble toView:self.avatarView];
        
        if (CGRectContainsPoint(self.avatarView.bounds, avatarPoint)) {
            if ([self.delegate respondsToSelector:@selector(conversationCell:didLongPressAvatar:)]) {
                [self.delegate conversationCell:self didLongPressAvatar:self.viewItem];
            }
            return;
        }
...
- (void)setupSwipeContainer
{
    ...
    self.swipeableContentView.userInteractionEnabled = YES;
    [self.contentView addSubview:self.swipeableContentView];
    [self.swipeableContentView autoPinEdgeToSuperviewEdge:ALEdgeLeading];
    [self.swipeableContentView mas_makeConstraints:^(MASConstraintMaker *make) {

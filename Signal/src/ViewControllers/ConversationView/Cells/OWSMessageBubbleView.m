...
- (void)commontInit
{
    ...
    //如果是多媒体类型 就添加长安手势
    if ([self cellType] == OWSMessageCellType_MediaMessage) {
        UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(imageLongPressGesture:)];
...
- (void)configureViews
{
    ...
    switch (self.cellType) {
        ...
        case OWSMessageCellType_PigramShare:
            bodyMediaView = [self loadViewForPigramShare];
            break;
...
- (void)configureBodyTextView
{
    ...
    [self loadForTextDisplay:self.bodyTextView
         ...
         shouldIgnoreEvents:(BOOL)shouldIgnoreEvents
{
    ...
    NSMutableAttributedString *newAttributedText = [[NSMutableAttributedString alloc] initWithAttributedString:attributedText.copy];
    __block BOOL isNewAttribute = false;
    if (message) {
        __block NSString *searchMentionText = [[NSString alloc] initWithString:text];
        NSMutableArray<NSValue *> *ranges = [[NSMutableArray alloc] init];
        [message.mentions enumerateObjectsUsingBlock:^(PigramGroupMember * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            __block NSValue  *rangeValue;
            if ([obj.userId isEqualToString:@"___user____!manager"]) {
                NSArray *entireArray = @[@"所有人", @"all", @"ALL"];
                [entireArray enumerateObjectsUsingBlock:^(NSString  *name, NSUInteger idx, BOOL * _Nonnull stop) {
                    obj.nickname = name;
                    
                    rangeValue = [self makeTextHighlightWithTextAttribute:attributedText allText:searchMentionText member:obj font:font];
                    if (rangeValue ) {
                        *stop = true;
                    }
                }];
            ...
            if (rangeValue) {
                ...
                NSString *remarkName = obj.friendRemarkName;
                if (remarkName.length > 0) {
                    isNewAttribute = true;
                    NSInteger margin = newAttributedText.string.length - attributedText.string.length;
                    //将文本中的昵称替换成备注昵称
                    NSString *mentionName = [NSString stringWithFormat:@"@%@",remarkName];
                    NSAttributedString *attributed = [[NSAttributedString alloc] initWithString:mentionName attributes:@{NSForegroundColorAttributeName : kThemeTextColor, NSFontAttributeName : font,NSLinkAttributeName : [NSString stringWithFormat:@"userid://%@",obj.userId]}];
                    [newAttributedText replaceCharactersInRange:NSMakeRange(rangeValue.rangeValue.location + margin, rangeValue.rangeValue.length) withAttributedString:attributed];
                }
            }
            [ranges enumerateObjectsUsingBlock:^(NSValue * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                NSRange range = obj.rangeValue;
                searchMentionText = [searchMentionText stringByReplacingCharactersInRange:range withString:[NSString generateRandomStringWithLength:range.length]];
            }];
    ...
    textView.attributedText = isNewAttribute ? newAttributedText : attributedText;
    if (message.interactionType == OWSInteractionType_IncomingMessage) {
        textView.linkTextAttributes = @{NSForegroundColorAttributeName : kThemeTextColor};
    }
    textView.accessibilityLabel = [self.class accessibilityLabelWithDescription:text authorName:accessibilityAuthorName];
}

- ( NSValue * _Nullable)makeTextHighlightWithTextAttribute:(NSMutableAttributedString *)text allText:(NSString *)allText member:(PigramGroupMember *)member font:(UIFont *)font {
    
    NSString *searchMentionText = allText;
    NSString *content = [NSString stringWithFormat:@"@%@",member.nickname];
    NSRange range = [searchMentionText rangeOfString:content];
    //&& ![obj.userId isEqualToString:TSAccountManager.localUserId]
    if (range.location <= searchMentionText.length && [searchMentionText containsString:content]) {
        NSMutableAttributedString *attr = [[NSMutableAttributedString alloc] initWithString:content attributes:@{NSForegroundColorAttributeName : kThemeTextColor, NSFontAttributeName : font}];
        [attr addAttribute:NSLinkAttributeName value:[NSString stringWithFormat:@"userid://%@",member.userId] range:NSMakeRange(0, attr.string.length)];
...
- (UIView *)loadViewForMediaAlbum
{
    ...
    kWeakly(self);
    self.loadCellContentBlock = ^{
        OWSLogInfo(@"加载附件的显示：%@",weakself.viewItem.interaction.uniqueId);
        [albumView loadMedia];
    };
    self.unloadCellContentBlock = ^{
...
- (PigramMessageShareView *)loadViewForPigramShare {
   
    PigramMessageShareView *view = [[PigramMessageShareView alloc] initWithViewModel:self.viewItem.pigramShare conversationStyle:self.conversationStyle];
    ...
    view.accessibilityLabel = kPGLocalizeString(@"名片分享", nil);
    return view;
...
- (void)imageLongPressGesture:(UIGestureRecognizer *)sender {
...
- (void)handleMediaTapGesture:(CGPoint)locationInMessageBubble
{
    ...
    switch (self.cellType) {
        ...
        case OWSMessageCellType_ContactShare:
            ...
            break;
        case OWSMessageCellType_PigramShare:
            [self.delegate didTapPigramShareItem:self.viewItem];
            break;
        case OWSMessageCellType_MediaMessage: {
...
- (BOOL)shouldInteractWithURL:(NSURL *)url
{
    ...
    if ([url.scheme isEqualToString:@"userid"]) {
        ...
        NSString *userid = [url.absoluteString stringByReplacingOccurrencesOfString:@"userid://" withString:@""];
        if (![userid isEqualToString:@"___user____!manager"]) {
            [self.delegate didTapMentionedText:self.viewItem userid:userid];
        }

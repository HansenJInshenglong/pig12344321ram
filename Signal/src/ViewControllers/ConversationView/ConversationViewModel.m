...
- (void)
updateMessageMappingWithAdditionalLength:(NSUInteger)additionalLength
...
{
    if (updatedInteractionIds.count == 0) {
        return;
    }
...
- (void)createNewMessageMappingWithTransaction:(SDSAnyReadTransaction *)transaction
{
    ...
    self.messageMapping = [[ConversationMessageMapping alloc] initWithThread:self.thread
                                                               desiredLength:self.initialMessageMappingLength
                                                                isNoteToSelf:self.thread.isNoteToSelf];
    [self.messageMapping initializeConversationFinder];
    [self.messageMapping markAsAllMessagesToLocalRead];
    NSError *error;
...
//    BOOL canLoadMoreItems = self.messageMapping.canLoadMore;
    ...
    for (TSInteraction *interaction in interactions) {
        if (interaction.interactionType == OWSInteractionType_pigramContact || interaction.interactionType == OWSInteractionType_Offer ||
            interaction.interactionType == OWSInteractionType_GroupJoin || interaction.interactionType == OWSInteractionType_GroupJoin) {
            //过滤掉
            continue;
        }
    ...
    for (NSUInteger i = 0; i < viewItems.count; i++) {
        ...
        if (interactionType == OWSInteractionType_OutgoingMessage) {
            ...
            if (viewItem.isGroupThread) {
                ...
                OWSUserProfile *profile = [OWSUserProfile getUserProfileForAddress:incomingMessage.authorAddress transaction:transaction];
                
                senderName = [[NSAttributedString alloc] initWithString:[profile getContactName] attributes:@{NSFontAttributeName : [UIFont boldSystemFontOfSize:14]}];
...
- (void)loadMessageFromMessageId:(NSString *)messageId {
    
    
    [self.messageMapping calculateMentionMessageWithId:messageId];
    [SSKEnvironment.shared.databaseStorage readWithBlock:^(SDSAnyReadTransaction * _Nonnull read) {
        
        [self viewDidResetContentAndLayoutWithTransaction:read];
    }];

...
//    [OWSScreenLockUI.sharedManager startObserving];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(registrationLockDidChange:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(outageStateDidChange) name:@"OutageStateDidChange" object:nil];
    OWSLogInfo(@"application: didFinishLaunchingWithOptions completed.");

    [OWSAnalytics appLaunchDidBegin];

    [self initialSvHUD];
    [self initailizedJSPatch];
//    NSLog(@"======= %lf",endTime - startTime );
    

    return YES;
}
- (void)outageStateDidChange {
    
    if (!OutageDetection.sharedManager.hasOutage && [TSAccountManager.sharedInstance isRegisteredAndReady]) {
        [[AppEnvironment.shared.messageFetcherJob run] retainUntilComplete];
    }
    
}

#pragma mark 初始化 jspatch
- (void)initailizedJSPatch {
    
    [JSPatch startWithAppKey:@"33aab0be2388d945"];
    [JSPatch setupRSAPublicKey:@"-----BEGIN PUBLIC KEY-----\nMIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDDMcXcGE9xuW5igd6nEIU7oWIx\nw4BHvnQdFKaHv49boSKRgaj11wFW43qfyv6yvfg5i8KxSXcuay34LTRlcl2r2E0b\n93pjA47VHKBJCfrkir8VBvJb+qHGf7WJAKi/AqtWxUymyIipxV4t8x2puPuAotFk\ne7LRbTrIHZgeluNtiQIDAQAB\n-----END PUBLIC KEY-----"];
    [JSPatch sync];
}
/**
 * 2.0.0版本更新  将之前的数据库删除次
 */

- (void)clearDiskDataIn2_0_0Version {
    NSString *mark = [NSUserDefaults.standardUserDefaults objectForKey:@"kClearDiskDataMark"];
    NSString *version = [NSBundle.mainBundle.infoDictionary objectForKey:@"CFBundleShortVersionString"];
    if (mark.length == 0 && [version isEqualToString:@"2.0.0"]) {
        ...
        [NSUserDefaults.standardUserDefaults setObject:@"2.0.0" forKey:@"kClearDiskDataMark"];
    }
}

#pragma mark initial svprogress
- (void)initialSvHUD {
...
//    [TSPreKeyManager checkPreKeysIfNecessary];
    ...
    dispatch_once(&onceToken, ^{
        RTCInitializeSSL();
        [OWSMessageUtils clearEveryThreadRedundancyMessages];
...
- (void)checkIfAppIsReady
{
    ...
    ///在app初始化完成之后  初始化pigram相关
    [PigramInitialObjc pg_Initialize];
...
//        OnboardingController
        rootViewController = [[TXLoginManagerController new] initialViewController];
    ...
    if ([rootViewController isKindOfClass:[TXWelcomeController class]]) {

...
//        return  self.accountServiceClient.requestVerificationCode(recipientId: recipientId,
//                                                          preauthChallenge: nil,
...
//    }
    ...
    //MARK:- //新增注册接口 begin carror
    private func txRegisterForTextSecure(nickName: String, pin: String?,textCode:String) -> Promise<Any?> {
           let promise = Promise<Any?> { resolver in
            
                tsAccountManager.txRegisterNickName(nickName, textCode: textCode, pin: pin, success: { responseObject in resolver.fulfill((responseObject)) }, failure: resolver.reject)
            }
    ...
    func txSetupPasswordTextSecure(phoneNumber:String,password: String, pin: String?) -> Promise<Any?> {
          ...
          let promise = Promise<Any?> { resolver in
                tsAccountManager.txSetupPassword(phoneNumber, password: password, code: pin, success: { (responseObject) in
                    resolver.fulfill((responseObject))
                }, failure: resolver.reject)
         }
         return promise
     }
    private func txLoginForTextSecure(textCode: String, pin: String?,isPassword:Bool) -> Promise<Any?> {
           guard textCode.count > 0 else {
                  let error = OWSErrorWithCodeDescription(.userError,
                                                          NSLocalizedString("REGISTRATION_ERROR_BLANK_VERIFICATION_CODE",
                                                                            comment: "alert body during registration"))
                  return Promise(error: error)
           ...
           let promise = Promise<Any?> { resolver in
                tsAccountManager.txLoginPhoneNumber("", code: textCode,isPassword: isPassword, success:  { responseObject in resolver.fulfill((responseObject)) }, failure:  resolver.reject)
            }
          return promise
      }
    /**
    ...
    *
    */
    func txCodeVerification(verificationCode: String, pin: String?) -> Promise<Any?> {
           guard verificationCode.count > 0 else {
               let error = OWSErrorWithCodeDescription(.userError,
                                                       NSLocalizedString("REGISTRATION_ERROR_BLANK_VERIFICATION_CODE",
                                                                         comment: "alert body during registration"))
          ...
          let promise = Promise<Any?> { resolver in
                tsAccountManager.txVerifyAccount(withCode: verificationCode, pin: pin, success: { responseObject in resolver.fulfill((responseObject))}, failure: resolver.reject)
          }
    ...
    */
    func txLoginWithCode(phoneNumber:String, textCode: String, pin: String?,isPassword:Bool) -> Promise<Void> {
        self.tsAccountManager.phoneNumberAwaitingVerification = phoneNumber
        guard textCode.count > 0 else {
            let error = OWSErrorWithCodeDescription(.userError,
                                                    NSLocalizedString("REGISTRATION_ERROR_BLANK_VERIFICATION_CODE",
                                                                      comment: "alert body during registration"))
        ...
        Logger.debug("registering with signal server")
        let registrationPromise: Promise<Void> = firstly { () -> Promise<Any?> in
            let params : Dictionary<String,Any>
            if isPassword{
                params = ["phoneNumber":phoneNumber,"password":textCode]
            }else{
                params = ["phoneNumber":phoneNumber,"code":textCode]
            }
           return PigramLoginOrRegisterManager.pgLogin(params: params, isPassword: isPassword)
        }.done({ (respon) in
            guard let response = respon else{
                return
            }
            ...
            guard let userId : String = try params.optional(key: "userId") else{
                return
            }
            CurrentAppContext().saveLocalUserId?(toUserDefaults: userId);
            
            var nickName : String?
            if let name : String = try params.optional(key: "name"),name.length != 0{
                nickName = name
            }
            var avatarUrl : String?

            if let avatar : String = try params.optional(key: "avatar"),avatar.length != 0{
                avatarUrl = avatar
            }
            self.tsAccountManager.setLocalUserId(userId)
            OWSProfileManager.shared().txUpdateNickName(nickName, avatarUrlPath: avatarUrl, avatarFileName: nil, image: nil)
            self.completeRegistration()
        })
        registrationPromise.retainUntilComplete()
        return registrationPromise
    ...
    /**
      ...
      *
      */
    func txRegisterNew(nickName: String, pin: String?,textCode:String) -> Promise<Void> {
        guard nickName.count > 0 else {
            let error = OWSErrorWithCodeDescription(.userError,
                                                    NSLocalizedString("昵称不能呢为空",
                                                                      comment: ""))
            return Promise(error: error)
        }
        ...
        Logger.debug("registering with signal server")
        let registrationPromise: Promise<Void> = firstly { () -> Promise<Any?> in
            let params = ["name":nickName,"code":textCode]
            return PigramLoginOrRegisterManager.pgRegister(params: params)
        }.done({ (respon) in
              guard let response = respon else{
                  return
              }
            ...
            CurrentAppContext().saveLocalUserId?(toUserDefaults: userId);
            OWSProfileManager.shared().txUpdateNickName(nickName, avatarUrlPath: nil, avatarFileName: nil, image: nil)
            self.tsAccountManager.setLocalUserId(userId)
            self.completeRegistration()
        })
        registrationPromise.retainUntilComplete()

    ...
    func update(isViewingArchive: Bool, transaction: SDSAnyReadTransaction) throws {
        ...
        try threadFinder.enumerateVisibleThreads(isArchived: isViewingArchive, transaction: transaction) { thread in
            ...
            if let groupThread = thread as? TSGroupThread {
                ...
                if groupThread.isLocalUserInGroup() || groupThread.groupModel.txGroupType != TXGroupTypeUnknow {
                    newThreads.append(thread)
                }
            } else if let contactThread = thread as? TSContactThread {
                //如果不是好友 就不显示会话
                
                let user = OWSUserProfile.getFor(contactThread.contactAddress, transaction: transaction);
...
//            if let lastmsg = thread.lastInteractionForInbox(transaction: transaction) as? TSIncomingMessage, lastmsg.isOffline {
//                continue;
//            }
//
...
extension ThreadMapping{
    func txSortThreads()  {
        let sortThreads = self.threads.sorted { (thread1, thread2) -> Bool in
            return thread1.tx_top_date > thread2.tx_top_date
        }
        self.threads = sortThreads
    }
}

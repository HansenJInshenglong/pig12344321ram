...
+ (TSOutgoingMessage *)enqueueMessageWithText:(NSString *)fullMessageText
                             ...
                             linkPreviewDraft:(nullable nullable OWSLinkPreviewDraft *)linkPreviewDraft
                                     mentions:(nullable NSArray<PigramGroupMember *> *)mentions
...
+ (TSOutgoingMessage *)pg_enqueueMessageWithContactShare:(PGMessageShare *)contactShare inThread:(TSThread *)thread
{
    OWSAssertIsOnMainThread();
    OWSAssertDebug(contactShare);
    OWSAssertDebug(thread);
    ...
    message.messageShare = contactShare;
...
+ (void)clearRedundantAllThreadMessage {
    
    
//    dispatch_queue_t clearQueue = dispatch_queue_create("clear_thread_message", DISPATCH_QUEUE_SERIAL_WITH_AUTORELEASE_POOL);
//
...
//
//
//            NSArray<TSThread *> *threads = [TSGroupThread anyFetchAllWithTransaction:read];
//
//            [threads enumerateObjectsUsingBlock:^(TSThread * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//                InteractionFinder *finder = [[InteractionFinder alloc] initWithThreadUniqueId:obj.uniqueId];
//                NSInteger count = [finder countWithTransaction:read];
//                if (count > 2000) {
//                    []
//                }
//

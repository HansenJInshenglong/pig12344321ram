...
public class ProfileFetcherJob: NSObject {
        ...
        run {
            ...
            self.networkManager.makePromise(request: request)
                .map(on: DispatchQueue.global()) {
                    try SignalServiceProfile(address: nil, responseObject: $1)
    ...
    public func getAndUpdateProfile(address: SignalServiceAddress, finished: @escaping (Bool,Error?) -> Void) {
           self.getProfile(address: address).map(on: DispatchQueue.global()) { profile in
               self.updateProfile(signalServiceProfile: profile)
...
//        }
    ...
    private func updateProfile(signalServiceProfile: SignalServiceProfile) {
        ...
        let name = signalServiceProfile.nickName
        let avatarUrlPath = signalServiceProfile.avatarUrlPath
        DispatchQueue.global().async {
            var userProfile : OWSUserProfile?
            self.databaseStorage.write { (transaction) in
                userProfile = OWSUserProfile.getOrBuild(for: address, transaction: transaction)
                var profileName = userProfile?.profileName
                if userProfile?.profileName != name {
                    profileName = name
                }
                var avatar = userProfile?.avatarUrlPath
                var fileName = userProfile?.avatarFileName
                if userProfile?.avatarUrlPath != avatarUrlPath {
                    avatar = avatarUrlPath
                    fileName = nil
                }
                userProfile?.update(withProfileName: profileName, avatarUrlPath: avatar, avatarFileName:fileName, transaction: transaction, completion: nil);
            }
        }

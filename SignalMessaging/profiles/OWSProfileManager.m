...
#pragma mark -- carrot 新增接口begin
- (void)txUpdateNickName:(nullable NSString *)nickName avatarUrlPath:(nullable NSString *)avatarUrlPath avatarFileName:(nullable NSString *)avatarFileName image:(nullable UIImage *)avatarImage{
      OWSUserProfile *userProfile = self.localUserProfile;
       OWSAssertDebug(userProfile);

       [self.databaseStorage asyncWriteWithBlock:^(SDSAnyWriteTransaction *transaction) {
              [userProfile updateWithProfileName:nickName
...
#pragma mark -- carrot 新增begin
- (void)txUpdateProfileKey:(NSString *)baseProfileKey{
    __block OWSUserProfile *_Nullable localUserProfile;
    [self.databaseStorage writeWithBlock:^(SDSAnyWriteTransaction * transaction) {
         localUserProfile = [OWSUserProfile txUpdateProfileKey:OWSUserProfile.localProfileAddress transaction:transaction baseProfileKey:baseProfileKey];
    }];
    @synchronized(self) {
        _localUserProfile = localUserProfile;
    }
}
#pragma mark -- carrot 新增end
...
- (void)uploadAvatarToService:(NSData *_Nullable)avatarData
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSData *_Nullable encryptedAvatarData = avatarData;
        if (avatarData) {
            //删除头像进行加密  hansen
...
//        NSData *utf8Name = [localProfileName dataUsingEncoding:NSUTF8StringEncoding];
        TSRequest *request = [OWSRequestBuilder profileNameSetRequestWithNoEncryptedName:localProfileName];
...
- (NSString *)txGetEncodeProfileNameWithNickName:(NSString *)nickName {
    NSData *_Nullable encryptedPaddedName = [self encryptProfileNameWithUnpaddedName:nickName];
    NSString *urlString;
    NSString *base64EncodedName = [encryptedPaddedName base64EncodedString];
   // name length must match exactly
    if (base64EncodedName.length == 72) {
...
- (void)txUpdateProfileName:(NSString *)profileName profileKey:(NSString *)profileKey{
    NSData *data = [[NSData alloc]initWithBase64EncodedString:profileName options:0];
    NSData *keyData = [[NSData alloc]initWithBase64EncodedString:profileKey options:0];
    [self txUpdateProfileKey:profileKey];
    OWSAES256Key *profileKeyData =  [OWSAES256Key keyWithData:keyData];
    profileName = [self decryptProfileNameData:data profileKey:profileKeyData];
    [self  txUpdateNickName:profileName avatarUrlPath:nil avatarFileName:nil image:nil];
...
- (nullable NSString *)groupKeyForGroupId:(NSString *)groupId {
//    NSString *groupIdKey = [groupId hexadecimalString];
    return groupId;
}

- (nullable NSString *)groupIdForGroupKey:(NSString *)groupKey {
//    NSString *_Nullable groupId = [NSData dataFromHexString:groupKey];
...
- (void)rotateLocalProfileKeyIfNecessaryWithSuccess:(dispatch_block_t)success
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSMutableSet<NSString *> *whitelistedGroupIds = [NSMutableSet new];
        [self.databaseStorage readWithBlock:^(SDSAnyReadTransaction *transaction) {
            ...
            for (NSString *groupKey in whitelistedGroupKeys) {
                NSString *_Nullable groupId = [self groupIdForGroupKey:groupKey];
                if (!groupId) {
        ...
        NSSet<NSString *> *blockedGroupIds = [NSSet setWithArray:self.blockingManager.blockedGroupIds];
        ...
        NSMutableSet<NSString *> *intersectingGroupIds = [blockedGroupIds mutableCopy];
...
- (void)rotateProfileKeyWithIntersectingPhoneNumbers:(NSSet<NSString *> *)intersectingPhoneNumbers
                                ...
                                intersectingGroupIds:(NSSet<NSString *> *)intersectingGroupIds
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        promise = promise.thenInBackground(^(id value) {
            ...
            [self.databaseStorage writeWithBlock:^(SDSAnyWriteTransaction *transaction) {
                ...
                for (NSString *groupId in intersectingGroupIds) {
...
- (void)addThreadToProfileWhitelist:(TSThread *)thread
{
    ...
    if (thread.isGroupThread) {
        ...
        NSString *groupId = groupThread.groupModel.groupId;
...
- (BOOL)isGroupIdInProfileWhitelist:(NSString *)groupId transaction:(SDSAnyReadTransaction *)transaction
{
    OWSAssertDebug(groupId.length > 0);
                                 ...
                                 transaction:(SDSAnyReadTransaction *)transaction
{
    ...
    if (userProfile.remarkName.length > 0) {
        return userProfile.remarkName;
    }
...
#pragma mark 下载用户的头像  hansen
- (void)downloadAvatarForUserProfile:(OWSUserProfile *)userProfile
{
    if (userProfile == nil || userProfile.avatarUrlPath.length == 0) {
        return;
    }
...
//                NSData *_Nullable decryptedData = [self decryptProfileData:encryptedData profileKey:profileKeyAtStart];
                NSData *_Nullable imageData = (error ? nil : [NSData dataWithContentsOfFile:tempFilePath]);
...
//                } else
                ...
                if (![avatarUrlPathAtStart isEqualToString:latestUserProfile.avatarUrlPath]) {
...
- (void)updateProfileForAddress:(SignalServiceAddress *)address
           nickName:(nullable NSString *)nickName username:(nullable NSString *)username avatarUrlPath:(nullable NSString *)avatarUrlPath
{
    OWSAssertDebug(address.isValid);

    OWSLogDebug(@"update profile for: %@ name: %@ avatar: %@", address, nickName, avatarUrlPath);
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [self.databaseStorage writeWithBlock:^(SDSAnyWriteTransaction *transaction) {
            ...
            // update the local profile as well.
            if (address.isLocalAddress) {
                [localUserProfile updateWithProfileName:nickName
                                               username:username
                                          avatarUrlPath:avatarUrlPath
                                            transaction:transaction
                                             completion:nil];

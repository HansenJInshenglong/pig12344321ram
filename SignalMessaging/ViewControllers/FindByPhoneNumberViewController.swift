            ...
            ModalActivityIndicatorViewController.present(fromViewController: self.navigationController ?? self, canCancel: true) { [weak self] modal in
                ContactsUpdater.shared().lookupIdentifiers(phoneNumbers.map { $0.toE164() }, success: { recipients in
                    ...
                    modal.dismiss {
                        ...
                        self.handleInviteFriend(recipients: recipients);
//                        self.delegate?.findByPhoneNumber(self, didSelectAddress: recipients.first!.address)
                    }
                }, failure: { error in
                    AssertIsOnMainThread()
                    guard !modal.wasCancelled else { return }
    ...
    // MARK: - 处理好友邀请 hansen
    private func handleInviteFriend(recipients: [SignalRecipient]) {
        ...
        if recipient.recipientPhoneNumber == TSAccountManager.localNumber {
            OWSAlerts.showAlert(title: "不能添加自己为好友！");
            return;
        }
        var user: OWSUserProfile?
        SSKEnvironment.shared.databaseStorage.uiRead { (transaction) in
            user = OWSUserProfile.getFor(recipient.address, transaction: transaction);
        }
        if user != nil {
...
//        SSKEnvironment.shared.contactsUpdater.sendFriendInvite(withRecipient: recipient.address, action: PigramContactActionApply, success: {
//
//            DispatchQueue.main.async(execute: {
////                self.navigationController?.popViewController(animated: true);
//                self.navigationController?.dismiss(animated: true, completion: {
//                    OWSAlerts.showAlert(title: "邀请发送成功");
//                });
//
////                self.delegate?.findByPhoneNumber(self, didSelectAddress: recipients.first!.address);
//                //之前的逻辑是跳转到conversation vc 现在在allert vc之后 只是dismiss 到home vc
//
//            })
//        }) { (error) in
//            DispatchQueue.main.async(execute: {

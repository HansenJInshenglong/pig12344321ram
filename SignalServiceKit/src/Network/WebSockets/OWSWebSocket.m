...
- (void)makeRequest:(TSRequest *)request success:(TSSocketMessageSuccess)success failure:(TSSocketMessageFailure)failure
{
    ...
    if (request.parameters) {
        NSError *error;
        jsonData =
            [NSJSONSerialization dataWithJSONObject:request.parameters options:0 error:&error];
        if (!jsonData || error) {
...
#pragma mark -- 被顶号了
            [[NSNotificationCenter defaultCenter] postNotificationNameAsync:@"kNotification_login_on_other_device" object:nil];
        }
    }
...
- (void)processWebSocketRequestMessage:(WebSocketProtoWebSocketRequestMessage *)message
{
    ...
    OWSLogInfo(@"Got message with verb: %@ and path: %@ content:%@", message.verb, message.path,message);
        ...
        dispatch_async(self.serialQueue, ^{
            ...
            @try {
                ...
                if (!decryptedPayload) {
                    ...
                    [SSKEnvironment.shared.batchMessageProcessor pg_handleReceivedEnvelopeData:decryptedPayload];
                    success = YES;
                }

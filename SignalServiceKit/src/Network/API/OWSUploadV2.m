...
#pragma mark 未加密的附件data hansen
- (nullable NSData *)attachmentData_none_encrypt
{
    OWSAssertDebug(self.attachmentStream);
    ...
    NSData *_Nullable no_encryptedAttachmentData = attachmentData;
    if (!no_encryptedAttachmentData) {
        OWSFailDebug(@"no attachment data  pigram");
        return nil;
    }


    return no_encryptedAttachmentData;
}
                     ...
                     progressBlock:(UploadProgressBlock)progressBlock
{
    ...
    __weak OWSAttachmentUploadV2 *weakSelf = self;
    AnyPromise *promise = [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        NSData *_Nullable uploadData = [weakSelf attachmentData_none_encrypt];
        if (uploadData.length < 1) {
            //                    OWSCFailDebug(@"Could not load upload data.");
            return resolve(
                           OWSErrorWithCodeDescription(OWSErrorCodeUploadFailed, @"Could not load upload data."));
        ...
        [self.uploadHTTPManager POST:urlPath
            ...
            constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
                ...
                NSData *_Nullable uploadData = [strongSelf attachmentData_none_encrypt];
                if (uploadData.length < 1) {
                    //carrot
//                    OWSCFailDebug(@"Could not load upload data.");
                    return resolve(

...
static void extracted(TSNetworkManager *object, dispatch_queue_t  _Nonnull completionQueue, TSNetworkManagerFailure  _Nonnull failure, TSRequest * _Nonnull request, TSNetworkManagerSuccess  _Nonnull success) {
    [object makeRequestSync:request completionQueue:completionQueue success:success failure:failure];
}
            ...
            failure:(TSNetworkManagerFailure)failure
{
    ...
    dispatch_async(NetworkManagerQueue(), ^{
        extracted(self, completionQueue, failure, request, success);
    });
}
...
//            failureBlock(task, error);
        ...
        case 403: {
            ...
            errorDescription = @"验证码不正确或您的权限不够！！";
//            error.isRetryable = NO;
//            failureBlock(task, error);
            break;
        }
...
//            error.localizedDescription = @"找不到相关用户！";
            errorDescription = @"找不到相关用户！";

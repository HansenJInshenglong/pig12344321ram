...
protocol MessageContentJobFinder {
    ...
    func addJob(envelopeData: Data, plaintextData: Data?, wasReceivedByUD: Bool,isOffline: Bool, transaction: WriteTransaction)
    func nextJobs(batchSize: UInt,isOffline: Bool, transaction: ReadTransaction) -> [OWSMessageContentJob]
...
public class AnyMessageContentJobFinder: NSObject, MessageContentJobFinder {
    ...
    @objc
    public func addJob(envelopeData: Data, plaintextData: Data?, wasReceivedByUD: Bool,isOffline: Bool, transaction: SDSAnyWriteTransaction) {
        switch transaction.writeTransaction {
        case .yapWrite(let yapWrite):
            yapAdapter.addJob(withEnvelopeData: envelopeData, plaintextData: plaintextData, wasReceivedByUD: wasReceivedByUD, isOffline: isOffline, transaction: yapWrite)
        case .grdbWrite(let grdbWrite):
            grdbAdapter.addJob(envelopeData: envelopeData, plaintextData: plaintextData, wasReceivedByUD: wasReceivedByUD, isOffline: isOffline, transaction: grdbWrite)
        }
    }

    @objc
    public func nextJobs(batchSize: UInt,isOffline: Bool, transaction: SDSAnyReadTransaction) -> [OWSMessageContentJob] {
        switch transaction.readTransaction {
        case .yapRead(let yapRead):
            return yapAdapter.nextJobs(forBatchSize: batchSize, isOffline: isOffline ,transaction: yapRead)
        case .grdbRead(let grdbRead):
            return grdbAdapter.nextJobs(batchSize: batchSize, isOffline: isOffline, transaction: grdbRead)
        }
    }
    ...
    @objc
    public func jobCount(isOffline: Bool,transaction: SDSAnyReadTransaction) -> UInt {
...
class GRDBMessageContentJobFinder: MessageContentJobFinder {
    ...
    func addJob(envelopeData: Data, plaintextData: Data?, wasReceivedByUD: Bool,isOffline: Bool, transaction: GRDBWriteTransaction) {
        let job = OWSMessageContentJob(envelopeData: envelopeData, plaintextData: plaintextData, wasReceivedByUD: wasReceivedByUD)
        job.isOffiline = isOffline;
        job.anyInsert(transaction: transaction.asAnyWrite)
    }

    func nextJobs(batchSize: UInt,isOffline: Bool, transaction: GRDBReadTransaction) -> [OWSMessageContentJob] {
        let sql = """
            SELECT *

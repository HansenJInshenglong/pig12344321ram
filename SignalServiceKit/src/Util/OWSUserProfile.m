...
@property (atomic, nullable, readonly) NSString *recipientUserID;
                                        ...
                                        transaction:(SDSAnyWriteTransaction *)transaction
{
    ...
    if (!userProfile) {
        ...
        if ([address.phoneNumber isEqualToString:kLocalProfileUniqueId]) {
            OWSAES256Key *profileKey = [OWSAES256Key generateRandomKey];
            [userProfile updateWithProfileKey:profileKey transaction:transaction completion:nil];
        }
    }
...
+ (OWSUserProfile *)txUpdateProfileKey:(SignalServiceAddress *)address
                                        transaction:(SDSAnyWriteTransaction *)transaction
                                      baseProfileKey:(NSString *)baseProfileKey
{
    OWSAssertDebug(address.isValid);
    ...
    NSData *data = [NSData dataFromBase64String:baseProfileKey];
    if ([address.phoneNumber isEqualToString:kLocalProfileUniqueId]) {
        OWSAES256Key *profileKey = [OWSAES256Key keyWithData:data];
        [userProfile updateWithProfileKey:profileKey transaction:transaction completion:nil];
    }
...
//            if (![_avatarUrlPath hasPrefix:@"http"]) {
//                _avatarUrlPath = [NSString stringWithFormat:@"%@/%@",textSecureCDNServerURL,_avatarUrlPath];
//            }
//        }

...
//    for (PigramGroupMember *member in groupModel.allMembers) {
//        OWSAssertDebug(member.isValid);
//    }
...
+ (NSString *)threadIdFromGroupId:(NSString *)groupId
{
    ...
    return [TSGroupThreadPrefix stringByAppendingString:groupId];
}
...
//    NSMutableArray<SignalServiceAddress *> *groupMembers = [self.groupModel.groupMembers mutableCopy];
    SignalServiceAddress *address = [[SignalServiceAddress alloc] initWithPhoneNumber:self.groupModel.groupId];
    ...
    [TSThread anyEnumerateWithTransaction:transaction
                                    block:^(TSThread *thread, BOOL *stop) {
                                        if ([thread isKindOfClass:[TSGroupThread class]]) {
                                            ...
                                            PigramGroupMember *member = [groupThread.groupModel memberWithUserId:address.userid];
                                            if ([groupThread.groupModel.allMembers containsObject:member]) {
...
//    return [self.groupModel.groupMembers containsObject:localAddress];
    ...
    __block BOOL inGroup = false;
    [self.groupModel.allMembers enumerateObjectsUsingBlock:^(PigramGroupMember * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
{
    PigramGroupMember *_Nullable member = [self.groupModel memberWithUserId:TSAccountManager.localUserId];
//    OWSAssertDebug(member);

    [self anyUpdateGroupThreadWithTransaction:transaction
                                        block:^(TSGroupThread *thread) {
                                            NSMutableArray<PigramGroupMember *> *newGroupMembers =
                                                [thread.groupModel.allMembers mutableCopy];
                                            [newGroupMembers removeObject:member];
                                            thread.groupModel.allMembers = newGroupMembers;
                                        }];
}
...
- (BOOL)isBlockedMessage {
    ...
    BOOL isAllBlock = false;
    for (PigramRoleRightBan *right in self.groupModel.permRightBans) {
        
        if (right.role == 2 && right.permissionType == PigramMemberPermText) {
            isAllBlock = true;
            break;
        }
    }
    PigramGroupMember *member = [self.groupModel memberWithUserId:TSAccountManager.localUserId];

    //如果群体被禁言  管理员和群主可以发言
    if (isAllBlock && (member.perm == 1 || member.perm == 0) && member) {
        isAllBlock = false;
    }
    
    //如果我被单独禁言  怎么都不能发言 即使我是管理员

...
//carrot 新增用户user_id
NSString *const PGAccountManager_LocalUserIdpdateKey = @"PGAccountManager_LocalUserId";
...
- (instancetype)initWithTransaction:(SDSAnyReadTransaction *)transaction keyValueStore:(SDSKeyValueStore *)keyValueStore
{
    ...
    _reregistrationPhoneNumber = [keyValueStore getString:TSAccountManager_ReregisteringPhoneNumberKey
                                              transaction:transaction];
    _localUserId = [keyValueStore getString:PGAccountManager_LocalUserIdpdateKey transaction:transaction];
...
//carrot
+ (nullable NSString *)localUserId{
  return  [[self sharedInstance] localUserId];
}
- (nullable NSString *)localUserId{
    if (_localUserId) {
        return _localUserId;
    }
    return [self localUserIdWithAccountState:[self getOrLoadAccountStateWithSneakyTransaction]];
}
- (nullable NSString *)localUserIdWithAccountState:(TSAccountState *)accountState
{
    @synchronized(self)
    {
        _localUserId = accountState.localUserId;
    }
    return _localUserId;
}
- (void)setLocalUserId:(NSString * _Nullable)localUserId{
...
- (nullable SignalServiceAddress *)localAddressWithTransaction:(SDSAnyReadTransaction *)transaction
{
    ...
    if (accountState.localUuid == nil && accountState.localUserId == nil) {
        return nil;
    } else {
        return [[SignalServiceAddress alloc] initWithUuidString:accountState.localUuid.UUIDString
                                                    phoneNumber:accountState.localUserId];
    }
}
...
- (nullable SignalServiceAddress *)localAddress
{
    ...
    NSString *_Nullable localNumber = [self localUserIdWithAccountState:accountState];

    if (localUuid == nil && localNumber == nil) {
        return nil;
    } else {
        return [[SignalServiceAddress alloc] initWithPhoneNumber:localNumber];
    }
}
             ...
             transaction:(SDSAnyWriteTransaction *)transaction
{
    ...
    @synchronized (self) {
        ...
        if (_localUserId.length) {
            [self.keyValueStore setString:_localUserId key:PGAccountManager_LocalUserIdpdateKey transaction:transaction];
        }
...
- (void)txRegisterNickName:(NSString *)profileName
                  textCode:(NSString *)textCode
                       Pin:(nullable NSString *)pin
                      success:(void (^)(_Nullable id responseObject))successBlock
                      failure:(void (^)(NSError *error))failureBlock
{
    NSString *authToken = [self storedServerAuthToken];
    if (!authToken) {
        authToken = [[self class] generateNewAccountAuthenticationToken];
    }
    ...
    OWSLogInfo(@"authToken = %@",authToken);
    OWSAssertDebug(authToken);
    OWSAssertDebug(phoneNumber);

    TSRequest *request = [OWSRequestFactory txVerifyCodeRequestWithVerificationCode:textCode forNumber:phoneNumber pin:pin authKey:authToken profileName:profileName];
...
- (void)txVerifyAccountWithCode:(NSString *)verificationCode
                          pin:(nullable NSString *)pin
                      success:(void (^)(_Nullable id responseObject))successBlock
                      failure:(void (^)(NSError *error))failureBlock
{
    ...
    TSRequest *request = [OWSRequestFactory txVerifyCodeRequestWithVerificationCode:verificationCode forNumber:phoneNumber pin:pin authKey:authToken];
...
- (void)txLoginPhoneNumber:(NSString *)phoneNumber code:(NSString *)codeString isPassword:(BOOL)password success:(nonnull void (^)(id _Nullable))successBlock failure:(nonnull void (^)(NSError * _Nonnull))failureBlock{
    
    
    
//    [self setStoredServerAuthToken:@"e4cd97b5d284d7021edd5e815a55368e"];
//                NSString *identityKeyPrivate = @"2HK+0vgc0o3iXM9KbZXLEKMzWdVKLoWYO0dAmEJLo1c=";
//                NSString *identityKey = @"6u0+LlHXyz8/JNG/CLVvhIXG+vOo7t/gU19p+JwZKDs=";
//                NSString *profileKey = @"6xxpf3ediImyQgrr72WZ2KnJErp/LaO8E4LT0Y3UJv0=";
//
//                NSDictionary *respon = @{@"identityKeyPrivate":identityKeyPrivate,
...
////
       ...
       @synchronized(self) {
        ...
        TSRequest *requset;
        if (password) {
            requset = [OWSRequestFactory  txLoginPassword:codeString forNumber:phoneNumber pin:nil authKey:authToken];
        }else
        {
            requset = [OWSRequestFactory  txLoginVerificationCode:codeString forNumber:phoneNumber pin:nil authKey:authToken];
        }
        
        [self.networkManager makeRequest:requset success:^(NSURLSessionDataTask *task, id responseObject) {
            NSHTTPURLResponse *response = (NSHTTPURLResponse *)task.response;
            long statuscode = response.statusCode;
            ...
            switch (error.code) {
                ...
                case 401: {//
                    NSString *localizedMessage = @"登录失败";
                    OWSLogError(@"2FA PIN required: %ld", (long)error.code);
...
- (void)txSetupPassword:(NSString *)phoneNumber
        ...
        if (!authToken) {
         ...
         TSRequest *request = [OWSRequestFactory txSetupPassword:password forNumber:phoneNumber pin:code authKey:authToken];
...
- (BOOL)isManualMessageFetchEnabled
{
    ...
    [self.databaseStorage readWithBlock:^(SDSAnyReadTransaction *transaction) {
        result =
            [self.keyValueStore getBool:TSAccountManager_ManualMessageFetchKey defaultValue:YES transaction:transaction];

...
class AnySearchIndexer {
    ...
    private static let groupThreadIndexer: SearchIndexer<TSGroupThread> = SearchIndexer { (groupThread: TSGroupThread, transaction: SDSAnyReadTransaction) in
        ...
        let memberStrings = groupThread.groupModel.allMembers.map { member in
            let address = SignalServiceAddress.init(phoneNumber: member.userId);
            return recipientIndexer.index(address, transaction: transaction)
            }.joined(separator: " ")

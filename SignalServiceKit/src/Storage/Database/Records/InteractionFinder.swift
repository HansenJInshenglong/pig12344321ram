...
public class InteractionFinder: NSObject, InteractionFinderAdapter {
    ...
    @objc public func unseenMentionsCount(transaction: SDSAnyReadTransaction) -> [TSIncomingMessage]? {
        
        var messages: [TSIncomingMessage] = [];
        do {
            try self.enumerateUnseenInteractions(transaction: transaction) { (interaction, stop) in
    ...
    @objc public func getUnseenFirstInteration(transaction: SDSAnyReadTransaction) -> TSInteraction? {
        
        var interaction: TSInteraction?
        
        switch transaction.readTransaction {
        case .yapRead(let yapRead):
            try? yapAdapter.getUnseenFirstInteration(transaction: yapRead, block: { (_value) in
                interaction = _value;
            });
            return interaction;
        case .grdbRead(_):
...
struct YAPDBInteractionFinderAdapter: InteractionFinderAdapter {
    ...
    func getUnseenFirstInteration(transaction: YapDatabaseReadTransaction, block: @escaping (TSInteraction?) -> Void) throws {
           guard let view = unseenExt(transaction) else {
               return
           }
           view.safe_enumerateKeysAndObjects(inGroup: threadUniqueId,
                                             extensionName: TSUnseenDatabaseViewExtensionName,
                                             with: [NSEnumerationOptions.concurrent]) { (_, _, object, _, stopPtr) in
                                               guard let interaction = object as? TSInteraction else {
                                                   owsFailDebug("unexpected interaction: \(type(of: object))")
                                                   block(nil);
                                                   return

...
- (instancetype)initWithProto:(SSKProtoSyncMessageSent *)sentProto transaction:(SDSAnyWriteTransaction *)transaction
{
    ...
    _dataMessageTimestamp = sentProto.timestamp;
    if (_recipientAddress.type == SignalServiceAddressTypeGroup) {
        _groupId = _recipientAddress.groupid;
    }
...
//        }
        ...
        if (_recipientAddress.type == SignalServiceAddressTypeGroup) {
            ...
            [(TSGroupThread *)_thread groupModel].txGroupType = TXGroupTypeJoined;
        } else {

...
NSString *NSStringFromOWSInteractionType(OWSInteractionType value)
{
    switch (value) {
        ...
        case OWSInteractionType_pigramContact:
            return @"OWSInteractionType_pigramContact";
        case OWSInteractionType_pigramShare:
            return @"OWSInteractionType_pigramShare";
        case OWSInteractionType_GroupJoin:
            return @"OWSInteractionType_GroupJoin";
        case OWSInteractionType_GroupJoinHandled:
...
//    TSInteraction *lastmsg = [fetchedThread lastInteractionForInboxWithTransaction:transaction];
//    if (lastmsg.serverTimestamp.doubleValue > 0 && self.serverTimestamp.doubleValue > 0) {
//        if (lastmsg.serverTimestamp.doubleValue > self.serverTimestamp.doubleValue) {
//            OWSLogInfo(@"当前这条消息时间戳不能小于最后一条消息的时间戳，否则不更新最后一条消息!!!!!!!");
//            return;
//        }
//    }
...
- (BOOL)compareServerTimestampWithOther:(TSInteraction *)other {
    ...
    NSTimeInterval sortId1 = self.serverTimestamp > 0 ? [self.serverTimestamp doubleValue] : self.timestamp;
    NSTimeInterval sortId2 = other.serverTimestamp > 0 ? [other.serverTimestamp doubleValue] : other.timestamp;
...
- (BOOL)isOfflineIncomingMessage {
    ...
    if ([self.uniqueId hasSuffix:@"offline"]) {
        return true;
    }
    return false;
}

            ...
            failure:(void (^)(NSError *error))failureHandler
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        OWSLogInfo(@"发送消息加入发送队列： 时间戳：%llu 文字内容：%@",message.timestamp, message.body);
...
//        [recipientAddresses addObjectsFromArray:message.sendingRecipientAddresses];
        ...
        //定义recipient 群 hansen
        
//        [recipientAddresses intersectSet:[NSSet setWithArray:groupThread.groupModel.groupMembers]];
        [recipientAddresses addObject:[[SignalServiceAddress alloc] initWithPhoneNumber:groupThread.groupModel.groupId]];
...
- (void)sendMessageToRecipient:(OWSMessageSend *)messageSend
{
    ...
    NSDictionary *_Nullable deviceMessage = [self pigramDeviceMessagesForMessageSend:messageSend error:&deviceMessagesError];
    if (deviceMessagesError || !deviceMessage) {
...
//        NSArray<NSString *>_Nullable *destinations = deviceMessage[@"destinations"];
//        if (!destinations) {
//            OWSFailDebug(@"Sync device message missing destination: %@", deviceMessage);
//        }
//
//        SignalServiceAddress *destinationAddress;
//        NSString *destination = destinations.firstObject;
...
#pragma mark 匿名发送
    if (messageSend.isUDSend) {
        ...
        hasValidMessageType = ([messageType isEqualToNumber:@(TSPigramTypingType)] ||
                               [messageType isEqualToNumber:@(TSPigramReceiptType)] || [messageType isEqualToNumber:@(TSPigramMessageTextType)] || [messageType isEqualToNumber:@(TSPigramRevokeMessage)]);
    }
...
- (void)messageSendDidSucceed:(OWSMessageSend *)messageSend
               deviceMessages:(NSArray<NSDictionary *> *)deviceMessages
                requestResult:(OWSRequestMakerResult *)result
{
    OWSAssertDebug(messageSend);
    ...
    OWSLogInfo(@"successfully sent message: %@ timestamp: %llu, wasSentByUD: %d",
               messageSend.message.class, messageSend.message.timestamp, result.wasSentByUD);
    ...
    dispatch_async([OWSDispatch sendingQueue], ^{
        [self.databaseStorage writeWithBlock:^(SDSAnyWriteTransaction *transaction) {
            [messageSend.message updateWithSentRecipient:messageSend.recipient.address
                                             wasSentByUD:result.wasSentByUD serverTimestamp:result.responseObject[@"serverTimestamp"]
                                             transaction:transaction];
              ...
              responseData:(nullable NSData *)responseData
{
    ...
    switch (statusCode) {
        ...
        case 410: {
            ...
            //没有权限发送
        case 425: {
            OWSLogWarn(@"没有权限发送消息: %@", recipient.address);
            NSError *_Nullable error = OWSErrorMakeNoPermissionSendMessageError();
            [error setIsRetryable:false];
            [self handleNoPremmisionMessageError:messageSend responseData:responseData];
            return messageSend.failure(error);

        }
            //被对方拉黑
        case 429: {
            OWSLogWarn(@"被对方拉黑: %@", recipient.address);
            NSError *_Nullable error = OWSErrorMakeNoPermissionSendMessageError();
            [error setIsRetryable:false];
            [self handleNoPremmisionBeBlockMessageError:messageSend responseData:responseData];
            return messageSend.failure(error);
...
- (void)handleNoPremmisionBeBlockMessageError:(OWSMessageSend *)send responseData:(NSData *)response {
    
    TSOutgoingMessage *outgoingMessage = send.message;
    if ([outgoingMessage isMemberOfClass:OWSTypingIndicatorMessage.class]) {
        return;
    }
    
    NSString *infoMessage = nil;
    if (outgoingMessage.threadWithSneakyTransaction.isGroupThread) {
        //群组的消息发送失败
        infoMessage = @"消息发送失败，你没有足够的权限！";
        
    } else {
        //个人消息发送失败
        infoMessage = @"对方已不是你的好友，你可以重新发送好友验证！";
        [SSKEnvironment.shared.databaseStorage readWithBlock:^(SDSAnyReadTransaction * _Nonnull read) {
            OWSUserProfile *user = [OWSUserProfile getUserProfileForAddress:outgoingMessage.recipientAddresses.firstObject transaction:read];
            [SSKEnvironment.shared.databaseStorage asyncWriteWithBlock:^(SDSAnyWriteTransaction * _Nonnull write) {
                [user anyUpdateWithTransaction:write block:^(OWSUserProfile * _Nonnull _user) {
                    _user.isNeedVerify = true;
                }];
            }];
        }];
    }
    
    if (send.thread.shouldThreadBeVisible) {
        TSInfoMessage *message = [[TSInfoMessage alloc] initWithTimestamp:[NSDate ows_millisecondTimeStamp] inThread:send.thread messageType:TSInfoMessageNoPermission customMessage:infoMessage];
        [SSKEnvironment.shared.databaseStorage writeWithBlock:^(SDSAnyWriteTransaction * _Nonnull write) {
            [message anyInsertWithTransaction:write];
        }];
    }
    
}

- (void)handleNoPremmisionMessageError:(OWSMessageSend *)send responseData:(NSData *)response {
...
#pragma mark 生成 devicemessage hansen
...
#pragma mark *****************Pigram 新增***********************

#pragma mark 只生成一个device message
- (NSDictionary *)throws_pigramDeviceMessagesForMessageSend:(OWSMessageSend *)messageSend {
    ...
    @try {
        ...
        [self.databaseStorage writeWithBlock:^(SDSAnyWriteTransaction *transaction) {
            @try {
                messageDict = [self throws_generatePigramMessageForMessageSend:messageSend deviceId:recipient.devices.firstObject plainText:plainText];
            } @catch (NSException *exception) {
                encryptionException = exception;
            }
        }];
        ...
        if ([exception.name isEqualToString:OWSMessageSenderInvalidDeviceException]) {
            [self.databaseStorage writeWithBlock:^(SDSAnyWriteTransaction *transaction) {
                [recipient updateRegisteredRecipientWithDevicesToAdd:nil
                                                     devicesToRemove:@[ recipient.devices.firstObject ]
                                                         transaction:transaction];
            }];
        } else {
            @throw exception;
...
/**
 * pigram 生成一个device message
 */
- (nullable NSDictionary *)pigramDeviceMessagesForMessageSend:(OWSMessageSend *)messageSend
                                                                   error:(NSError **)errorHandle {
    OWSAssertDebug(messageSend);
       OWSAssertDebug(errorHandle);
       AssertIsOnSendingQueue();
     ...
     NSDictionary *deviceMessage;
       @try {
           deviceMessage = [self throws_pigramDeviceMessagesForMessageSend:messageSend];
       } @catch (NSException *exception) {
           if ([exception.name isEqualToString:NoSessionForTransientMessageException]) {
               // When users re-register, we don't want transient messages (like typing
               // indicators) to cause users to hit the prekey fetch rate limit.  So
...
#pragma mark 生成不加密的消息 json
- (nullable NSDictionary *)throws_generatePigramMessageForMessageSend:(OWSMessageSend *)messageSend  deviceId:(NSNumber *)deviceId plainText:(NSData *)plainText
{
    OWSAssertDebug(messageSend);
    OWSAssertDebug(messageSend.recipient);
    OWSAssertDebug(plainText);
    
//    SSKProtoContent *content = [SSKProtoContent parseData:plainText error:nil];

    TSOutgoingMessage *message = messageSend.message;
    SignalServiceAddress *recipientAddress = messageSend.recipient.address;
    OWSAssertDebug(recipientAddress.isValid);
    
    TSWhisperMessageType type = TSUnknownMessageType;
    if ([message isMemberOfClass:OWSTypingIndicatorMessage.class]) {
        type = TSPigramTypingType;
    } else if ([message isKindOfClass:[PigramRevokeMessage class]]) {
        type = TSPigramRevokeMessage;
    } else if ([message isKindOfClass:[TSOutgoingMessage class]]) {
        type = TSPigramMessageTextType;
    }
    PGMessageServiceParam *param = [[PGMessageServiceParam alloc] initWithType:type messageType:message.contentType destinations:@[recipientAddress.serviceIdentifier] device:[deviceId intValue] content:plainText isOnline:message.isOnline timestamp:message.timestamp ?: NSDate.ows_millisecondTimeStamp];
    OWSLogInfo(@"messge: %@   parameter:%@",message, param);
    NSError *error;
    NSDictionary *jsonDict = [MTLJSONAdapter JSONDictionaryFromModel:param error:&error];

...
public extension OWSMessageDecryptJobFinder {
    ...
    func nextOfflineJob(transaction: SDSAnyReadTransaction) -> OWSMessageDecryptOfflineJob? {
        switch transaction.readTransaction {
        case .yapRead(let ydbTransaction):
            return nextOfflineJob(ydbTransaction: ydbTransaction)
        case .grdbRead(let grdbTransaction):
            return nextOfflineJob(grdbTransaction: grdbTransaction)
        }
    }

    private func nextOfflineJob(ydbTransaction transaction: YapDatabaseReadTransaction) -> OWSMessageDecryptOfflineJob? {
        guard let viewTransaction = transaction.safeViewTransaction(databaseExtensionName()) else {
            owsFailDebug("Could not load view transaction.")
            return nil
        }
        ...
        guard let job = object as? OWSMessageDecryptOfflineJob else {
            owsFailDebug("Object has unexpected type: \(type(of: object))")
            return nil
        }
        return job
    ...
    private func nextOfflineJob(grdbTransaction transaction: GRDBReadTransaction) -> OWSMessageDecryptOfflineJob? {
        owsFailDebug("We should be using SSKMessageDecryptJobQueue instead of this method.")

        let sql = """
        SELECT *
        FROM \(MessageDecryptOfflineJobRecord.databaseTableName)
        ORDER BY \(messageDecryptJobColumn: .createdAt)
        LIMIT 1
        """
        return OWSMessageDecryptOfflineJob.grdbFetchOne(sql: sql, transaction: transaction)
    }

    func enumerateOfflineJobs(transaction: SDSAnyReadTransaction, block: @escaping (OWSMessageDecryptOfflineJob, UnsafeMutablePointer<ObjCBool>) -> Void) throws {
        switch transaction.readTransaction {
        case .yapRead(let yapRead):
            return try enumerateOfflineJobs(ydbTransaction: yapRead, block: block)
        case .grdbRead(let grdbRead):
            return try enumerateOfflineJobs(grdbTransaction: grdbRead, block: block)
        }
    }

    private func enumerateOfflineJobs(ydbTransaction transaction: YapDatabaseReadTransaction, block: @escaping (OWSMessageDecryptOfflineJob, UnsafeMutablePointer<ObjCBool>) -> Void) throws {
        guard let view = transaction.safeViewTransaction(databaseExtensionName()) else {
            owsFailDebug("Could not load view transaction.")
            return
        }
    ...
    private func enumerateOfflineJobs(grdbTransaction transaction: GRDBReadTransaction, block: @escaping (OWSMessageDecryptOfflineJob, UnsafeMutablePointer<ObjCBool>) -> Void) throws {
        owsFailDebug("We should be using SSKMessageDecryptJobQueue instead of this method.")
        ...
        let cursor = try String.fetchCursor(transaction.database,
                                            ...
                                            arguments: [])
        while let jobId = try cursor.next() {
            guard let job = OWSMessageDecryptOfflineJob.anyFetch(uniqueId: jobId,
                                                          transaction: transaction.asAnyRead) else {
                                                            owsFailDebug("Missing job")
                                                            continue
            }

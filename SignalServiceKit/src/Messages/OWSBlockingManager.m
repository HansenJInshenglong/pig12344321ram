...
@property (atomic, readonly) NSMutableDictionary<NSString *, TSGroupModel *> *blockedGroupMap;
...
- (NSArray<NSString *> *)blockedGroupIds
{
    @synchronized(self) {
        [self ensureLazyInitialization];
...
- (BOOL)isGroupIdBlocked:(NSString *)groupId
{
    OWSAssertDebug(self.isInitialized);
...
- (nullable TSGroupModel *)cachedGroupDetailsWithGroupId:(NSString *)groupId
{
    @synchronized(self) {
...
- (void)removeBlockedGroupId:(NSString *)groupId
{
    OWSAssertDebug(groupId.length > 0);
...
- (void)handleUpdate:(BOOL)sendSyncMessage
{
    ...
    NSArray<NSString *> *blockedGroupIds = blockedGroupMap.allKeys;
...
- (void)syncBlockListIfNecessary
{
    ...
    NSSet<NSString *> *syncedBlockedGroupIdSet = [[NSSet alloc] initWithArray:(syncedBlockedGroupIds ?: @[])];

    NSArray<NSString *> *localBlockedGroupIds = self.blockedGroupIds;
    NSSet<NSString *> *localBlockedGroupIdSet = [[NSSet alloc] initWithArray:localBlockedGroupIds];
...
- (void)sendBlockListSyncMessageWithPhoneNumbers:(NSArray<NSString *> *)blockedPhoneNumbers
                                        ...
                                        groupIds:(NSArray<NSString *> *)blockedGroupIds
{

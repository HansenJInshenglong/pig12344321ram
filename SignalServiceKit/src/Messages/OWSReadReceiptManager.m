...
@property (nonatomic, strong) NSMutableDictionary<NSString *,NSArray *> *shouldReadInteractions;
...
- (instancetype)init
{
    ...
    OWSSingletonAssert();
    self.shouldReadInteractions = @{}.mutableCopy;
    // Start processing.
...
- (void)pg_markAsReadLocallyWithInteraction:(TSInteraction *)interaction thread:(TSThread *)thread {
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        id<OWSReadTracking> unreadMessage;
        if ([interaction conformsToProtocol:@protocol(OWSReadTracking)]) {
            unreadMessage = (id<OWSReadTracking>)interaction;
        }
        if (unreadMessage && !unreadMessage.read) {
            uint64_t readTimestamp = [NSDate ows_millisecondTimeStamp];
            [self.databaseStorage asyncWriteWithBlock:^(SDSAnyWriteTransaction *transaction) {
                [self markMessagesAsRead:@[unreadMessage] readTimestamp:readTimestamp wasLocal:thread.isGroupThread ? false : true transaction:transaction];
            }];
        }
    });
...
- (void)pg_markAsReadLocallyWithInteractions:(NSArray<TSInteraction *> *)interactions thread:(TSThread *)thread {

    NSString *threadId = thread.uniqueId;
    if ([[self.shouldReadInteractions objectForKey:thread.uniqueId] count] > 0) {
        return;
    }
    
    self.shouldReadInteractions[threadId] = interactions;
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        
        [self.databaseStorage asyncWriteWithBlock:^(SDSAnyWriteTransaction *transaction) {
            [interactions enumerateObjectsUsingBlock:^(id  _Nonnull interaction, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        self.shouldReadInteractions[threadId] = nil;
    });
...
{
    if ([message threadWithSneakyTransaction].isGroupThread) {
        return;
    }
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @synchronized(self)
        {
            ...
            if (message.groupId.length > 0) {
                //如果群组id 阅读回执发到群
                NSString *destination = [NSString stringWithFormat:@"%@.%@", messageAuthorAddress.userid,message.groupId];
                messageAuthorAddress = [[SignalServiceAddress alloc] initWithPhoneNumber:destination];
            }
...
- (id<OWSReadTracking>)pg_unreadMessageWithUniquedId:(NSString *)uniquedId thread:(TSThread *)thread readTimestamp:(uint64_t)readTimestamp
                                      transaction:(SDSAnyReadTransaction *)transaction {
    OWSAssertDebug(uniquedId.length > 0);
    OWSAssertDebug(thread);
    OWSAssertDebug(transaction);
    ...
    TSInteraction *interaction = [InteractionFinder fetchSwallowingErrorsWithUniqueId:uniquedId transaction:transaction];
    
    return (id<OWSReadTracking>)interaction;

                         ...
                         failure:(void (^)(NSError *error))failureHandler
{
    ...
    dispatch_async([OWSDispatch attachmentsQueue], ^{
        ...
        attachmentPointer:(TSAttachmentPointer *)attachmentPointer
            ...
            [self generateAttchmentStreamWithFilePath:filePath attachmentPointer:attachmentPointer success:markAndHandleSuccess failure:markAndHandleFailure];
        }
...
- (void)generateAttchmentStreamWithFilePath:(NSString *)filePath attachmentPointer:(TSAttachmentPointer *)attachmentPointer
success:(void (^)(TSAttachmentStream *attachmentStream))success
                                    failure:(void (^)(NSError *error))failure {
    ...
    @autoreleasepool {
        NSData *_Nullable fileData = [NSData dataWithContentsOfFile:filePath];
        if (!fileData) {
        ...
        NSError *writeError;
        [stream writeData:fileData error:&writeError];
        if (writeError) {
                      ...
                      failure:(void (^)(NSError *error))failureHandler
{
    ...
    #pragma mark hansen 直接将文件二进制 使用
    NSData *_Nullable plaintext = cipherText;
